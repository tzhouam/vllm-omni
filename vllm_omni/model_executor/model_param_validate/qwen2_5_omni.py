...
def validate_model_params(
    ...
    *,
    ...
    # Qwen2.5-Omni stage-specific suggestions.
    for i, (sampling_params, engine_args, stage) in enumerate(zip(sampling_params_list, engine_args_list, stage_list)):
        ...
        if model_stage == "talker":
            ...
            if sampling_params.max_tokens is not None:
                try:
                    if sampling_params.max_tokens < sampling_params_list[0].max_tokens * 2:
                        errors.append(
                            _fmt(
                                stage,
                                "max_tokens",
                                sampling_params.max_tokens,
                                "Expected max_tokens >= thinker's max_tokens * 2 for talker stage.",
                            )
                        )
                except Exception:
                    errors.append(
                        _fmt(
                            ...
                            "max_tokens",
                            sampling_params.max_tokens,
                            "Expected an int with max_tokens >= thinker's max_tokens * 2 for talker stage.",
                        )
                    )
        ...
        if model_stage == "code2wav":
            ...
            # code2wav should have max_tokens larger than talker's * 640
            if "max_num_batched_tokens" in engine_args:
                try:
                    max_num_batched_tokens = int(engine_args["max_num_batched_tokens"])
                    if max_num_batched_tokens < 640 * sampling_params_list[1].max_tokens:
                        errors.append(
                            _fmt(
                                stage,
                    ...
                    errors.append(
                        _fmt(
                            ...
                            max_num_batched_tokens,
                            (
                                "Expected an int with max_num_batched_tokens >= 640 * talker's max_tokens "
                                "for code2wav stage as " + str(640 * sampling_params_list[1].max_tokens)
                            ),
                        )
                    )

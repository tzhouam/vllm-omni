...
100-sample node-placement profile measured p50/p95 0.671/1.454 s for 2 s
audio, with 403 CPU and 335 GPU rows (median component RTF 0.335). This
qualifies only the tested vocoder component; full co-resident streaming,
speech quality and sustained behavior remain open.
A [real generated-code 25-frame window](../../benchmarks/edge_harness/results/e2e_expansion_20260924/evidence/qwen_tts_vocoder_qualcomm/s25/real_code_window/README.md)
also passed local ONNX/eager parity at 1.02e-6 relative L2 and matched its
full FP32 decoder segment at 7.26e-8 relative L2. The corresponding S25
GPU-requested inference returned an unsaturated waveform at 1.153% relative
L2 / 38.76 dB SNR versus ONNX CPU. This tests one generated-code window,
not mobile talker/vocoder co-residency or speech quality.
...
relative L2 versus local ONNX CPU on one synthetic fixture. Its 100-sample
profile attributed all 728 rows to NPU, but p50/p95 was 3.662/3.676 s for
0.16 s audio (median component RTF 22.89). This artifact therefore fails
the playable-stream latency gate. Neither changes M2 support
...
For M2 on HX370, a [fresh AMD NPU cut-100 Code2Wav handoff](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/qwen_tts_amd_npu_cut100_waveform/README.md) found one early real-weight prefix that passes downstream waveform checks on three fixed two-frame inputs: one retained export fixture and two windows from a CPU-generated code stream. Separate VitisAI sessions placed one NPU node each, and the unchanged CPU suffix produced waveforms 0.0589%/0.00414%/0.01225% relative L2 from their corresponding CPU controls; zeroing the boundary changed them 4.86%/3.60%/2.24%. The CPU split reproduced each full short-graph waveform bitwise. This corrects the overly broad earlier statement that every tested NPU prefix fails the waveform gate: the later cut-486 and full mixed graph still fail, at 9.689% and 15.696% waveform relative L2 for their specific artifacts. The first cut-100 NPU session took 340.787 s to create; 20 alternating CPU-only fixed-segment repeats measured full vocoder 1.528/1.569 s and CPU suffix 1.515/1.584 s p50/p95 for 0.16 s of audio. A later 72-context-frame CPU window differed 3.51% from its matching longer-history CPU segment, so stateful chunk correctness and playable streaming remain unqualified. M2 remains component-level for the AMD NPU route until multiple prompts, explicit chunk state, listening quality, paired whole-request timing, admission/cancel and sustained resource gates pass.

The [later generated-code history sweep and HX370 NPU handoff](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/qwen_tts_chunk_history/README.md) refine that M2 blocker. On the same later two-frame window, CPU waveform error against the long 25-frame decode fell from 3.5124% with 72 history frames to 0.0001556% with all 95. A pinned 95-history ONNX export passed eager/ORT parity, and a matching VitisAI prefix actually executed one NPU node. Its captured boundary through the unchanged CPU suffix produced a waveform 0.01212% relative L2 from the long CPU decode. The NPU session took 348.460 s to create, while 20 alternating CPU-only repeats put the unchanged suffix near 2.0 s p50 for 2.0 s audio. This repairs one fixture's chunk-history numerical mismatch; it does not qualify bounded rolling state, listening quality or complete-request benefit. M2 remains component-level for the AMD NPU route.

The [117-frame generated-code rolling audit](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/qwen_tts_rolling_history/README.md) then tested the same fixed 95-history/two-new-frame shape after its first passing chunk. CPU waveform error against the full FP32 decode was 0.5310% at frame 97, 0.7093% at 103 and 2.0214% at 109; the all-prior-history controls stayed within 0.00015%. This fixes the scope of the NPU component pass to its measured frame-95 input and reveals a rolling numerical blocker on the same utterance. M2 now needs bounded decoder state/cache with a validated history contract before a live NPU+CPU stream, followed by complete-request quality, timing, memory, cancellation and power gates. No NPU ran in this rolling audit.

The [pinned 117-frame Qwen3-TTS incremental-cache check](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/qwen_tts_incremental_cache/README.md) identifies the source decoder's truncated-window replay as the main CPU rolling error on this fixture. Joined waveform error is 1.5163% with two-frame chunks and 0.9640% with 25-frame chunks against the full FP32 decode; increasing downstream context to 117 frames does not change it. The new CPU x-vector async-chunk path appends to each transformer layer's sliding KV and retains bounded convolution/downstream state. Its integrated two-/25-frame outputs differ 1.68e-6/1.01e-6 relative L2 from full decode; physical first-layer KV is 71 frames after 117 frames seen. This repairs a CPU codec-state contract on one generated stream, not the NPU graph, complete request, playback or M2 acceptance.

The [same 117-frame Code2Wav stream on RTX 5090 Laptop CUDA](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/qwen_tts_cuda_incremental/README.md) exposes the same truncated-history defect on WSL: the previous eager CUDA path differs 0.9640% joined waveform relative L2 with 25-frame chunks and 46.2251% with two-frame chunks from full FP32 decode. Exact per-layer sliding KV reduces those errors to 1.18e-6 and 1.46e-6, and the segmented wrapper's stateful eager dispatcher reproduces the 25-frame exact result. The source now selects this cache for CUDA x-vector async chunks while retaining stateless graph and ICL paths. Four post-first up-to-25-frame exact chunks measured 42.9/58.8 ms p50/p95 on one uncontrolled run. A separate [short live Omni run](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/qwen_tts_cuda_exact_live/README.md) completed four serial text-to-audio requests, including one 92-frame medium utterance across the attention window, and shut down both stages. One measured short request delivered 2.80 s of audio in 0.603 s; a deliberate slow consumer incurred one simulated underrun. The numerical run remains component C and the new live run is scoped P. Neither establishes playback, listening quality or sustained GPU profile, so M2 acceptance remains open.
...
For M4, a [late SigLIP attention NPU cut](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/minicpmo_amd_npu_vision/README.md) moved the real BF16 graph to vision layer 27 after the earlier layer-one suffix failure. On three synthetic images the measured HX370 NPU attention output plus unchanged CPU BF16 MLP/post-layernorm stayed within 0.867–0.925% of the all-BF16 tensor, but the real resampler output differed 1.462–1.556% and still misses the provisional full-vision embedding gate. The full NPU layer differs 1.494–1.663% after resampling. One NPU node per image was observed; session build took 317.65 s. This remains component C with no image-answer quality, transfer-inclusive benefit, admission or complete-request validation.
...
For M4 on HX370, a newer [MiniCPM-o isolated projection check](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/minicpmo_amd_npu_projection/README.md) places the split after the CPU speech-head state update. Its A16W8 speech-token projection ran on VitisAI and preserved the CPU top token on 20/20 same-activation calls, with maximum relative L2 0.861% against FP32 CPU. That repairs the earlier full-graph cache error only by keeping cache computation on CPU. The isolated NPU projection measured 0.338 ms p50 against 0.057 ms for the FP32 CPU projection before cross-OS transfer, so the per-token split is not eligible as a default execution plan. Representative calibration, a coarser beneficial NPU stage and a full multimodal request remain open; this is component C, not M4 E2E.

For InternVLA on the same HX370, the [Cosmos Conv13 batch-one recovery](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/internvla_amd_npu_batch1/README.md) changes the M4 evidence boundary. The former six-frame A16W8 NPU graph produced a valid frame 0 but corrupted frames 1–5; six one-frame calls placed one NPU partition each and passed the provisional 1% numerical gate on two synthetic fixtures after calibration included all ramp frames. Replaying its output through the unchanged CPU encoder suffix yielded final-latent error 0.807%/0.562% on ramp/pattern, and the real CPU policy's synthetic action tensor changed 0.876% on pattern. Same-process Windows boundary timing was 14.468 ms NPU versus 23.778 ms FP32 CPU p50. A later [live Omni whole-policy run](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/internvla_omni_amd_npu_live/README.md) used the real checkpoint, CPU encoder prefix/suffix and six NPU Conv13 calls per synthetic request. It passed one warmup, 20 complete actions at 4.720/4.750 s p50/p95, explicit 16 GiB RAM admission and cancellation, but the actions differ 3.176% relative L2 from the separate CPU reference and there is no demonstrated full-request speed benefit. M4 advances to experimental synthetic policy execution on this NPU route, not task or release qualification. Real-observation tolerance, physical action metadata, paired benefit and sustained power remain unmeasured; the six-frame compiler/quality failures remain documented as artifact-specific failures.
...
A later [native Windows HX370 joint Omni policy run](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/internvla_joint_npu_radeon/README.md) changes that specific matrix cell to an experimental synthetic pass. One admitted stage kept the BF16 action policy and FP32 Cosmos prefix on CPU, executed six batch-one Conv13 calls on AMD NPU, then ran the FP32 Cosmos suffix in an isolated DirectML worker requested on Radeon 890M adapter 1. After one warmup, 20/20 serial synthetic observation-to-action requests passed at 4.525/4.683 s complete-request p50/p95; public entry, undersized admission refusal and in-flight cancellation/ledger cleanup also passed. Raw placement profiles retain one VitisAI node and 357 DirectML plus 34 CPU suffix nodes; the Radeon adapter identity is inferred from separate adapter enumeration. The joint action differs 3.188% relative L2 from the separate CPU action without a task tolerance. The separate CPU+Radeon p50 was 4.575 s, so no paired benefit is established. M4 remains experimental and opt-in pending real-observation reference actions, physical action metadata, paired full-request benefit, loading peak, concurrency/restart and sustained power/thermal tests.
...
| M4b | InternVLA-A1 | Strict-load synthetic policy forwards pass on WSL and native Windows CPU/RTX. Bounded Omni whole-policy stages pass on WSL CPU, WSL CPU+Radeon Cosmos, native Windows CPU, native Windows CPU+Radeon Cosmos, experimental native Windows CPU+AMD NPU Conv13, and experimental native Windows CPU+AMD NPU Conv13+Radeon suffix jointly. The joint route passed 20/20 synthetic actions, public entry, explicit shared-RAM admission/refusal and in-flight cancellation, with 4.525/4.683 s complete-request p50/p95 and 3.188% action relative L2 versus separate CPU. Neither task quality nor paired full-request benefit is established; the split is opt-in. This is not robot-task qualification. | Reference action agreement on real observations, physical units/order/step time, observation age and deadlines; paired full-request split benefit, post-cancel restart, loading peak, concurrency, power and thermal behavior. |
...
An HX370 [MiniCPM-o vision-stage NPU probe](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/minicpmo_amd_npu_vision/README.md) adds a coarser M4a candidate than the isolated speech-token projection. The real-weight fixed 448-pixel four-layer FP32 ONNX boundary matched FP32 Torch at 1.23e-6 relative L2. Quantizing all four layers placed 20 VitisAI nodes per call but missed the provisional 1% FP32 numerical gate at 1.048%. Keeping layer four FP32 on CPU placed 15 VitisAI nodes per call and passed that calibration-fixture FP32 gate at 0.911%; isolated 20-call NPU p50/p95 was 234.95/250.78 ms versus 567.05/576.19 ms for the same QDQ graph on CPU. Reusing the selective artifact on an independent image failed at 6.864% relative L2 versus FP32, with NPU still within 0.0492% of QDQ CPU. Two-image recalibration brought both included images below 0.926% versus FP32, but a third unseen image still failed at 1.579%. The first selective calibration output differed 1.425% from checkpoint BF16, and the unquantized 27-layer transformer-block FP32 export differed 5.311% from BF16. A measured three-layer NPU cut placed 15 VitisAI nodes per call and took 156.81/176.67 ms p50/p95 over 20 isolated calls after 13.65 s session creation, but its unseen third image failed the 1% FP32 gate at 1.592%. Replaying captured NPU tensors through unchanged BF16 layers 4–27 and post-layernorm differed 9.276%/7.660%/9.077% from the all-BF16 vision source on red/blue/third; the unquantized FP32 boundary alone differed 8.576%/6.672%/7.576%. The corresponding four-layer NPU replay also differed 9.183%/7.747%/9.096%, so moving the cut earlier did not repair downstream quality. A BF16-preserving one-layer graph with FP32 boundary casts also executed one VitisAI NPU node per call on three fixed images, but red/blue/independent-gradient outputs all missed the provisional 1% gate at 1.182%/1.249%/1.270% relative L2 versus BF16 Torch. The unchanged BF16 layers 2–27/post-layernorm replay differed 21.035%/16.875%/9.136%. The 321.30 s session creation and one call per input are not a repeated latency profile. An attention-residual bisection of that BF16 layer preserved the prior full NPU outputs bitwise and put one VitisAI node per image on HX370. Attention-residual errors were 0.709%/0.821%/0.846%, and a BF16 CPU MLP replay yielded 0.625%/0.682%/0.813% full-layer error on three synthetic images, within the provisional 1% component gate. The unchanged BF16 CPU vision suffix still differed 11.034%/9.003%/6.819% after post-layernorm; neither task quality nor transfer-inclusive benefit is established. A later FP32 last-layer CPU control passed the provisional full-embedding gate at 0.832–0.912% on three synthetic images, but the exact HX370 VitisAI graph missed it after the BF16 postnorm/resampler at 1.519–1.551%. Per-channel affine correction trained on two images left the held-out gradient at 1.413%. A separate FP32 CPU resampler with the unchanged BF16 SigLIP output already missed the gate on red at 1.082%, so that exact FP32 artifact was not compiled on NPU. A fixed-shape BF16-preserving whole-resampler wrapper then reproduced source Torch bitwise and ran as one VitisAI node per image after an 85.30 s session build, but its measured embeddings differed 3.516%/3.114%/3.101% relative L2 from the exact BF16 reference on red/blue/gradient. These one-off NPU calls provide no handoff-inclusive benefit claim; [raw FP32 and replay evidence](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/minicpmo_amd_npu_vision/README.md). A traced NPU graph preserved the original final output bitwise and measured postnorm at 0.756%/0.830%/0.828% relative L2, but the exact BF16 CPU projection from that measured tensor still missed the embedding gate at 2.153%/2.019%/2.014%. A custom ONNX ReferenceEvaluator control diverged 71.946% on red and is not a valid numerical reference; native ORT 1.30 CPU lacks BF16 MatMul(13), so export versus provider drift remains unresolved. M4a's AMD NPU cell remains NOT E2E; separate export and NPU execution error or establish a downstream image-task tolerance, then require independent representative validation and a complete request before an Omni placement.

For M2, a [current-source exact-state WSL RTX 5090 Laptop live profile](../../benchmarks/edge_harness/results/e2e_expansion_20260925/evidence/qwen_tts_cuda_exact_live20/README.md) completed one warmup plus 20 serial medium text-to-audio requests through both Omni CUDA stages. The measured requests produced ordered finite PCM with no arrival-simulated deficit, at nearest-rank TTFA 104.9/117.2 ms and complete-request wall 1.542/1.649 s p50/p95; output duration ranged 7.12–8.72 s. One pinned ASR pass exactly transcribed the separate warmup audio, and a post-abort short request completed. A deliberately slow consumer still incurred one simulated 40.6 ms gap, the earlier 1/180 measured deficit remains in the historical profile, and this 31 s measured window did not test a late-audio fence, sound-device playback or sustained thermal behavior. M2 remains incomplete; the next gate is a 30-minute exact-state run plus playback, late-cancel and broader quality checks before mobile stage placement.
...
The 2026-09-23 [Spark GGUF Omni follow-up](../../benchmarks/edge_harness/results/e2e_expansion_20260923/evidence/spark_omni_llamacpp/README.md) adds a separate native Windows CPU/Radeon 890M whole-session option. Its pinned 1.7B Q4_K_M artifact passed two named complete text requests and 20 serial inventory requests per device through `StageRuntime`/`StagePool`; public `AsyncOmni` short requests passed on both devices. It verifies CPU or Vulkan1 placement, explicit 4 GiB host reservation, terminal event/acknowledgement and cancellation drain. A [2026-09-24 restart audit](../../benchmarks/edge_harness/results/e2e_expansion_20260924/evidence/spark_omni_windows_restart/README.md) confirmed the server had begun the long request before cancellation on each placement, then a fresh StageRuntime with the same artifact returned Paris under a new worker generation with no stale output or reservation. This does not change the vLLM M0 acceptance or establish M1 mobile generation. Its next gates are incremental output, broader token/quality parity, loading/device-memory peaks, concurrency and sustained power/thermal behavior.

The [native Windows RTX Spark GGUF follow-up](../../benchmarks/edge_harness/results/e2e_expansion_20260924/evidence/spark_omni_windows_rtx/README.md) runs that same 1.7B Q4_K_M artifact through an Omni whole-session stage on RTX Vulkan0, with 29/29 layers offloaded. Two named requests, 20/20 serial inventory requests, a public `AsyncOmni` request, context refusal and fresh-stage restart after confirmed in-flight cancellation passed. Its 0.294/0.302 s complete-request p50/p95 is specific to the 2,463-prompt-token/five-output-token serial workload and cannot be compared directly to the earlier Spark 4B BF16 vLLM CUDA results. Token streaming, broad quality, VRAM/load peaks, concurrency and sustained power remain open; this does not establish mobile M1 support.

A [native Windows Qwen3.8-27B NVFP4 attempt](../../benchmarks/edge_harness/results/e2e_expansion_20260924/evidence/qwen38_omni_windows_rtx/README.md) did not reach inference despite loading all three weight shards. The installed Windows vLLM 0.29+cu134 wheel failed during Marlin post-load scale permutation with an unsupported PTX toolchain error under RTX driver 610.71/CUDA UMD 13.3. NVIDIA's PTX compatibility guidance makes a driver/toolchain mismatch likely; an upgraded-driver or compatible-wheel control is still needed before rerunning the 512-token text/image cases. This failure constrains the tested Windows route, not the previously passed WSL checkpoint path.

The separate official Qwen3.8-27B FP8 checkpoint now has a [scoped native-Windows Omni RTX text/image pass and 20-request serial profile per modality](../../benchmarks/edge_harness/results/e2e_expansion_20260924/evidence/qwen38_omni_windows_rtx/README.md). The installed Windows wheel lacks an SM120 CUTLASS block-FP8 operator, so an explicit Omni-owned Triton linear backend was added and verified. The 0.75 GPU-memory cap correctly refused the 512-token KV requirement; 0.80 with 12 GiB declared host offload admitted the run. A public AsyncOmni text/image check and same-engine abort/recovery also passed: the canceled long request emitted its terminal abort marker, cleared request state and was followed by a correct fresh request. This is a different artifact/backend from NVFP4 and covers only short text plus a synthetic red image. The next gates remain a server-started cancellation boundary, separate-stage restart, loading-peak admission, longer multimodal and quality cases, plus sustained power/thermal behavior.

The [native Windows RTX Qwen3-TTS Omni hybrid follow-up](../../benchmarks/edge_harness/results/e2e_expansion_20260924/evidence/qwen_tts_windows_rtx/README.md) extends the existing whole-session CrispASR stage to the optional discrete-GPU route. The RTX Vulkan0 talker/codec and CPU FP32 predictor produced two named complete WAVs, 20 identical serial measured outputs at 3.153/3.231 s p50/p95 for 2.72 s audio, and public AsyncOmni audio. Pinned Whisper tiny.en transcribed the first sentence exactly; the RTX PCM differs from the Radeon PCM, so the timings are not a same-output acceleration comparison. Separate 8 GiB host-RAM and 4 GiB GPU-VRAM reservations, a prelaunch GPU-budget refusal, in-flight cancellation and fresh-stage restart passed; the original Radeon route passed a regression probe. M2 still needs playable chunks, RTF below 1 for the declared workload, broader speaker quality, measured VRAM/load peaks, concurrency and an underrun-free sustained run. This Vulkan+CPU route does not qualify mobile NPU+GPU co-residency.

The [Qwen3-TTS Omni hybrid follow-up](../../benchmarks/edge_harness/results/e2e_expansion_20260923/evidence/qwen_tts_omni_hybrid/README.md) adds one complete-request stage for the native Windows HX370 CPU+Radeon 890M route. A pinned Q8_0 talker and F16 codec produced two named PCM outputs exactly matching the prior standalone backend; one warmup plus 20 serial requests measured 3.298/3.336 s p50/p95 for 2.56 s of audio. Public `AsyncOmni` audio, cancellation drain and explicit 8 GiB host admission passed. A separate [in-flight cancellation audit](../../benchmarks/edge_harness/results/e2e_expansion_20260924/evidence/qwen_tts_omni_windows_restart/README.md) confirmed that the request reached server prefill before cancellation; a fresh stage with the same artifacts and hybrid placement emitted the identical PCM under a new worker generation, without stale output or a retained reservation. This is progress toward M2 integration, not M2 acceptance: RTF remains above 1, and playable chunk streaming, same-session continuation, loading/iGPU peaks, broader voice quality and 30-minute thermal behavior remain open. No NPU participated.

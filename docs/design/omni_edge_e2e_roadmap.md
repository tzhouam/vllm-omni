...
| M4b | InternVLA-A1 | Strict-load synthetic policy forwards pass on WSL and native Windows CPU/RTX. Bounded Omni whole-policy stages pass on WSL CPU, WSL CPU+Radeon Cosmos, native Windows CPU, native Windows CPU+Radeon Cosmos and experimental native Windows CPU+AMD NPU Conv13 with shared-RAM admission, terminal action metadata and in-flight cancellation. The NPU route has a measured 3.176% synthetic action difference and no demonstrated full-request benefit; it is not enabled by default. Public CPU/hybrid entrypoints pass where measured. This is not robot-task qualification. | Reference action agreement on real observations, physical units/order/step time, observation age and deadlines; paired full-request NPU benefit, post-cancel restart, loading peak, concurrency, power and thermal behavior. |

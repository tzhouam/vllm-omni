...
"""Tests for the AR-Diffusion DreamZero validation harness (metrics / artifacts / config).
...
from benchmarks.ar_diffusion import artifacts, metrics
from benchmarks.ar_diffusion.dreamzero_config import ar_diffusion_config_for_dreamzero
from benchmarks.ar_diffusion.parity_check import compare
...
def test_config_derivation_matches_window():
    ...
    # window_chunks == local_attn_size (matches ARDiffusionModelRunner._preallocate_kv_cache,
    # which overwrites chunk_size to frame_seqlen but preserves window_chunks).
    cfg = ar_diffusion_config_for_dreamzero(num_frame_per_block=3, frame_seqlen=220, local_attn_size=21)
...
def test_config_window_is_frame_granular():
    ...
    cfg = ar_diffusion_config_for_dreamzero(num_frame_per_block=4, frame_seqlen=10, local_attn_size=21)
...
def test_config_rejects_full_attention():
    with pytest.raises(ValueError):
        ar_diffusion_config_for_dreamzero(num_frame_per_block=3, frame_seqlen=10, local_attn_size=-1)
...
def test_kv_memory_plateau(tmp_path):
    from benchmarks.ar_diffusion.dreamzero_config import ar_diffusion_config_for_dreamzero
    from benchmarks.ar_diffusion.profile import kv_memory_plateau, plot_memory

    cfg = ar_diffusion_config_for_dreamzero(num_frame_per_block=1, frame_seqlen=16, local_attn_size=2)

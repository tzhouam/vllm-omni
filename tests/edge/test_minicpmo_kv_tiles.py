...
def test_kv_projection_cuda_profile_bucket_is_bounded_and_tiled():
    hidden = np.broadcast_to(np.zeros((1, 1, 1152), dtype=np.float32),
                             (10, 1056, 1152))
    calls = []

    def project(tile, index, valid):
        calls.append((index, valid))
        return tile[..., :2]

    output = run_tiled_projection(hidden, project, output_width=2,
                                  max_logical_tokens=16384)
    assert output.shape == (10, 1056, 2)
    assert len(calls) == 11
    assert calls[-1] == (10, 320)

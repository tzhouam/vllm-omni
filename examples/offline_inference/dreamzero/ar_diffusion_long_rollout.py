...
"""Long DreamZero rollout in one session — to exercise AR-Diffusion KV eviction.
...
past the window and the model (and AR-Diffusion) must evict — letting us check that AR-Diffusion's
paged eviction stays parity-exact with the model-local path under real eviction.

KV-off vs KV-on is toggled by the AR_DIFFUSION_KV_ENABLE env var (same as the export):

    AR_DIFFUSION_KV_ENABLE=1 CUDA_VISIBLE_DEVICES=0 HF_HOME=/models \\
      python examples/offline_inference/dreamzero/ar_diffusion_long_rollout.py \\

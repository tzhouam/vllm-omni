...
A separate [HX370 AMD NPU Code2Wav attempt](evidence/qwen_tts_amd_npu_vocoder/README.md) tested the real-weight 25-frame and pinned two-frame FP32 ONNX exports on native Windows. Both matched retained ORT CPU waveforms within 1.05e-6 relative L2, but AMD VitisAI EP 1.8.63.0 aborted during graph compilation with an LLVM ArrayRef assertion before any NPU node or waveform ran. The cell remains NOT E2E; this failure is specific to the tested graphs and provider build.

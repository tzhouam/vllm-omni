...
"""Profiling utilities for AR-Diffusion DreamZero (plan §3).

``kv_memory_plateau`` is runnable without weights — it drives a ``ARDiffusionKVCache`` over
...
def kv_memory_plateau(
    ...
    *,
    ...
    """Drive a ``ARDiffusionKVCache`` over ``num_chunks`` and record pool occupancy.
...
def plot_memory(path, profile: dict) -> Path:
    ...
    ax.plot(profile["used_blocks"], label="pool blocks used (AR-Diffusion)")
    ...
    ax.set_title("AR-Diffusion KV pool occupancy (bounded by window)")
    ax.legend()
    fig.tight_layout()

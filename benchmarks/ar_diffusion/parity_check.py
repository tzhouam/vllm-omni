...
PSNR/SSIM/LPIPS.
    ...
    python -m benchmarks.ar_diffusion.parity_check \\
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="DreamZero AR-Diffusion KV parity check (KV-off vs KV-on)")
    ap.add_argument("--model", default="GEAR-Dreams/DreamZero-DROID")
    ap.add_argument("--deploy-off", required=True, help="deploy config with AR-Diffusion KV disabled (model-local)")
    ap.add_argument("--deploy-on", required=True, help="deploy config with AR-Diffusion KV enabled")
